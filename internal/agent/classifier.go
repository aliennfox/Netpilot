...
func configKeywords() []string {
	return []string{
		...
		// 真机暴露 (2026-04-27): "切到延迟最低的节点" 类用户 query 之前 fall through 到
		// 默认 Diagnose-only, LLM 看不到 switch_node 自然干瞪眼。 这里覆盖切节点的高频简写。
		"切到", "切回", "切去", "换到", "换回", "切节点", "换节点",
		"切最", "换最", "选最", "用最", // "切到最快/最低延迟" 等动作+排名组合
		"导入订阅", "添加订阅", "更新订阅", "删除订阅", "移除订阅",
		"应用模板", "套用模板", "套个模板",
		"添加节点", "导入节点", "新建节点",
		"配链", "搭链", "建链", "做链", "走链", "链式代理", "链路",
		"per-app", "白名单", "黑名单",
		"应用走", "走代理", "走直连",
	}
}
